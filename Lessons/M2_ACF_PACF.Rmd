...
inflow_data <- cbind(my_date,raw_inflow_data[,3:6]) #cbind stands for column bind
head(inflow_data)
...
```{r}
ts_inflow_data <- ts(inflow_data[,2:5],start = c(1931,1), frequency = 12) 
#Because the data is a monthly data, we don't put date.
#Also, the frequency is 12 because it is a monthly data.
ts_inflow_data
...
autoplot knows I want to play with time series data, and automatically considers x as date. 
...
HP1_acf= Acf(ts_inflow_data[,4], lag.max = 60)
# This is less confusing because we know the lag 0 = 1 (?)

HP1_pacf= Pacf(ts_inflow_data[,4], lag.max = 60, plot = TRUE)
